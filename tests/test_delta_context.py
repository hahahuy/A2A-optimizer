...
class TestMessageSavings(unittest.TestCase):
    ...
    def test_message_savings_correct_counts_and_pct(self):
        base = make_msgs("a", "b", "c")
        cp = self.mgr.checkpoint(base)
        current = base + make_msgs("d")
        stats = self.mgr.message_savings(cp, current)
        self.assertEqual(stats["full_message_count"], 4)
        self.assertEqual(stats["delta_count"], 1)
        self.assertEqual(stats["saved_count"], 3)
        self.assertAlmostEqual(stats["saved_message_pct"], 75.0)

    def test_message_savings_zero_pct_no_savings(self):
        base = make_msgs("a")
        cp = self.mgr.checkpoint(base)
        stats = self.mgr.message_savings(cp, base)
        self.assertEqual(stats["full_message_count"], 1)
        self.assertEqual(stats["delta_count"], 0)
        self.assertEqual(stats["saved_count"], 1)
        self.assertAlmostEqual(stats["saved_message_pct"], 100.0)


class TestDeltaContextManagerMaxCheckpoints(unittest.TestCase):
    def test_max_checkpoints_constructor_param(self):
        mgr = DeltaContextManager(max_checkpoints=5)
        self.assertEqual(mgr._max_checkpoints, 5)

    def test_max_checkpoints_zero_raises_value_error(self):
        with self.assertRaises(ValueError):
            DeltaContextManager(max_checkpoints=0)

    def test_max_checkpoints_negative_raises_value_error(self):
        with self.assertRaises(ValueError):
            DeltaContextManager(max_checkpoints=-1)

    def test_cache_stays_bounded(self):
        mgr = DeltaContextManager(max_checkpoints=3)
        for i in range(5):
            mgr.checkpoint(make_msgs(f"msg_{i}"))
        self.assertLessEqual(len(mgr._cache), 3)

    def test_oldest_checkpoint_evicted(self):
        mgr = DeltaContextManager(max_checkpoints=3)
        cp0 = mgr.checkpoint(make_msgs("msg_0"))
        cp1 = mgr.checkpoint(make_msgs("msg_1"))
        cp2 = mgr.checkpoint(make_msgs("msg_2"))
        mgr.checkpoint(make_msgs("msg_3"))
        self.assertNotIn(cp0.checkpoint_id, mgr._cache)
        self.assertIn(cp1.checkpoint_id, mgr._cache)
        self.assertIn(cp2.checkpoint_id, mgr._cache)

    def test_recently_used_checkpoint_not_evicted(self):
        mgr = DeltaContextManager(max_checkpoints=3)
        cp0 = mgr.checkpoint(make_msgs("msg_0"))
        mgr.checkpoint(make_msgs("msg_1"))
        mgr.checkpoint(make_msgs("msg_2"))
        mgr.checkpoint(make_msgs("msg_0"))
        mgr.checkpoint(make_msgs("msg_3"))
        self.assertIn(cp0.checkpoint_id, mgr._cache)

    def test_default_max_checkpoints_is_1000(self):
        mgr = DeltaContextManager()
        self.assertEqual(mgr._max_checkpoints, 1000)
