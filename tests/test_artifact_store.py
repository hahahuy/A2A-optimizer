...
class TestArtifactStoreLRU(unittest.TestCase):
    ...
    def test_lru_cascade_multiple_evictions_on_large_item(self):
        store = ArtifactStore(max_bytes=50)
        cids = []
        for i in range(5):
            cids.append(store.store(f"{'x' * 9}{i}".encode()))  # 10 bytes each
        self.assertEqual(store.count, 5)
        self.assertEqual(store.total_bytes, 50)

        # Store an 11-byte item — 50+11=61 > 50; evicts LRU items until total <= 50
        # Evicts cids[0] (61-10=51>50), then cids[1] (51-10=41<=50) → 2 evictions
        new_cid = store.store(b"y" * 11)

        self.assertEqual(store.count, 4)
        self.assertLessEqual(store.total_bytes, 50)
        with self.assertRaises(KeyError):
            store.retrieve(cids[0])
        with self.assertRaises(KeyError):
            store.retrieve(cids[1])
        self.assertEqual(store.retrieve(new_cid).cid, new_cid)

    def test_near_max_size_item_evicts_all_smaller_items(self):
        store = ArtifactStore(max_bytes=30)
        small_cids = []
        for i in range(3):
            small_cids.append(store.store(f"{'a' * 8}{i}".encode()))  # 9 bytes each
        self.assertEqual(store.total_bytes, 27)

        # 25-byte item forces eviction of all three 9-byte items
        big_cid = store.store(b"b" * 25)

        self.assertEqual(store.count, 1)
        self.assertEqual(store.total_bytes, 25)
        self.assertEqual(store.retrieve(big_cid).cid, big_cid)
