...
class TestMCPAdapterThreadSafety(unittest.TestCase):
    def test_concurrent_tools_register_no_race(self):
        """Concurrent tools_register_request calls produce consistent _schema_id."""
        adapter = MCPAdapter()
        results = []
        errors = []
        lock = threading.Lock()

        tools_a = [{"name": "tool_a", "description": "Tool A"}]
        tools_b = [{"name": "tool_b", "description": "Tool B"}]

        def register(tools):
            try:
                adapter.tools_register_request(tools, ttl=3600)
                with lock:
                    results.append(adapter._schema_id)
            except Exception as exc:
                errors.append(exc)

        threads = [
            threading.Thread(target=register, args=(tools_a,)),
            threading.Thread(target=register, args=(tools_b,)),
            threading.Thread(target=register, args=(tools_a,)),
            threading.Thread(target=register, args=(tools_b,)),
        ]
        for t in threads:
        ...
        self.assertEqual(errors, [], f"Concurrent register raised: {errors}")
        # Every result must be a valid sha256: string (no corruption, no None)
        for r in results:
            self.assertIsNotNone(r)
            self.assertTrue(r.startswith("sha256:"), f"Invalid schema_id: {r!r}")

    def test_concurrent_tools_list_response_no_exception(self):
        """tools_list_response reads _schema_id under lock — no exceptions under concurrency."""
        adapter = MCPAdapter()
        # Pre-register so _schema_id is set
        tools = [{"name": "t", "description": "T"}]
        adapter.tools_register_request(tools)
        errors = []

        def list_response():
            try:
                adapter.tools_list_response(tools, use_ref=True)
            except Exception as exc:
                errors.append(exc)

        threads = [threading.Thread(target=list_response) for _ in range(10)]
        for t in threads:
            t.start()
        for t in threads:
            t.join()

        self.assertEqual(errors, [], f"Concurrent list_response raised: {errors}")

    def test_schema_id_lock_exists(self):
        """MCPAdapter has a threading.Lock for _schema_id."""
        import threading as _threading
        adapter = MCPAdapter()
        self.assertIsInstance(adapter._schema_id_lock, type(_threading.Lock()))
